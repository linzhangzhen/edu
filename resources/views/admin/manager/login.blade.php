...
<div class="loginWraper">
  <div id="loginform" class="loginBox">
    @if(!empty($errors->first('errorinfo')))
      <div style="height:20px;background-color: #ff0000;">{{$errors->first('errorinfo')}}</div>
    @endif
    @if(!empty($errors->first('mg_name')))
      <div style="height:20px;background-color: #ff0000;">{{$errors->first('mg_name')}}</div>
    @endif
    @if(!empty($errors->first('password')))
      <div style="height:20px;background-color: #ff0000;">{{$errors->first('password')}}</div>
    @endif
    @if(!empty($errors->first('verify_code')))
      <div style="height:20px;background-color: #ff0000;">{{$errors->first('verify_code')}}</div>
    @endif
    <form class="form form-horizontal" action="{{url('admin/manager/login')}}" method="post">
      ...
      <div class="row cl">
        ...
        <div class="formControls col-xs-8">
          <input id="mg_name" name="mg_name" type="text" placeholder="账户" class="input-text size-L" value="{{old('mg_name')}}">
        </div>
      </div>
      <div class="row cl">
        <label class="form-label col-xs-3"><i class="Hui-iconfont">&#xe60e;</i></label>
